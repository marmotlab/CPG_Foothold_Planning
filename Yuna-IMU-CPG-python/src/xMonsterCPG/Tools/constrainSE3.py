...
# AB: expresses frame A w.r.t frame B
    ...
    #Set SE3 tranforms
    GW = SE3(cpg['G'], np.zeros([3]))        # ground in world
    WG = GW.T

    BW = SE3(cpg['pose'], np.zeros(3))      # body in world
    WB = BW.T

    GB = np.dot(GW, BW.T)                # ground in body
    BG = GB.T
    ...
    for leg in range(6):
        erStart = np.dot(BG[0:3,0:3] , (np.dot(rot_matrix , rG[0:3,leg]) - rG[0:3,leg]))   # feet correction distance in body frame

        er1 = np.array([0,0,erStart[2],1])  # get the distance is z axis
