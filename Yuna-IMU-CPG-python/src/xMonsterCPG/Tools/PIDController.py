...
class PIDController:
  ...
  def update(self, dt, error, upd):
    ...
    self.compP[:,upd] = np.clip(self.kP * error[:,upd],a_min = -self.limP,a_max=self.limP);

    if np.array_equal(np.sign(error[:,upd]), np.sign(self.prevError[:,upd])):
      self.compD[:,upd] = np.clip((self.error[:,upd] - self.prevError[:,upd])/self.dt * self.kD,a_min=-self.limD,a_max=self.limD);
    else:
      self.compD[:,upd] = 0;
    ...
    self.integral[:,upd] = self.integral[:,upd] + self.error[:,upd] * self.dt;
    self.integral = self.integral - self.integral * self.alpha;
    self.compI = np.clip(self.integral * self.kI,a_min=-self.limI,a_max=self.limI);
    self.out = np.clip(self.compP + self.compI + self.compD,a_min=-self.limTot,a_max=self.limTot);
