...
class HexapodKinematics(object):
    ...
    def getLegPositions_leg(self, index, angles):
        # The input is the angles with the shape of [1.18]
        # The output is the xyz coordinates of the end effectors in world coordinate
        # Shape of Output is [3,6]
    ...
    def getLegFrames_leg(self, index, angles):
        # The input is the angles with the shape of [1.18]
        # The output is the xyz coordinates of the end effectors in world coordinate
        # Shape of Output is [3,6]
    ...
    def getLegIK_leg(self, index, xd):
        ...
        if index == 1:
            ee_pos_objective2 = hebi.robot_model.endeffector_position_objective(xd[:, 1])
            angles[0:3] = self.rfLeg.solve_inverse_kinematics(guess[3:6], ee_pos_objective2)
        if index == 2:
            ee_pos_objective3 = hebi.robot_model.endeffector_position_objective(xd[:, 2])
            angles[0:3] = self.lmLeg.solve_inverse_kinematics(guess[6:9], ee_pos_objective3)
        if index == 3:
            ee_pos_objective4 = hebi.robot_model.endeffector_position_objective(xd[:, 3])
            angles[0:3] = self.rmLeg.solve_inverse_kinematics(guess[9:12], ee_pos_objective4)
        if index == 4:
            ee_pos_objective5 = hebi.robot_model.endeffector_position_objective(xd[:, 4])
            angles[0:3] = self.lbLeg.solve_inverse_kinematics(guess[12:15], ee_pos_objective5)
        if index == 5:
            ee_pos_objective6 = hebi.robot_model.endeffector_position_objective(xd[:, 5])
            angles[0:3] = self.rbLeg.solve_inverse_kinematics(guess[15:18], ee_pos_objective6)
